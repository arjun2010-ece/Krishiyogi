## Q1. Explain multiple processes and ports.
...
## Q2. How can we run a node app in multiple process?
...
## Q3. ```node app.js``` , can we fire this cmd multiple times to create multiple processes?
...
## Q4. How does PM2 or node.js cluster module decide how many processes to run on my CPU cores?
...
## Q5. Connection pool has a size of 10, then does it run 10 db process running on db server or on backend server, clarify ?
...
## Q6. If you deploy your app across 4 CPU cores using PM2 or the cluster module, and your code defines a max: 10 database connection pool: since this scaling is within a single server running multiple processes, is it a trap or bad? Should we provision multiple servers instead, and not use PM2 or the cluster module to run multiple processes on a single machine ?
